...
def Corte(v1,v2):
    ...
    while i>0:
        if i==len(v1) and v1[-1][0]==v2[-1][0]:
            break
        i-=1 #logo no inicio ja temos v1 decrementado, pois o tamanho da lista conta o indice 0
        if v1[i-1][0]<=v2[-1][0]:
            v1=v1[:i]
            break
...
while True:
    ...
    if(a=="1"):
        ...
        med = filedialog.askopenfile(parent=root,mode="r",title="Arquivo de medição(.csv) com decimal vírgula(,) e separador ponto e vírgula (;)")
        if med is None:
            exit(1)
...
####################################################################################################################################################
        ...
        med = filedialog.askopenfile(parent=root,mode="r",title="Arquivo de medição(.csv) com decimal virgula e separador ponto e vírgula (;)")
        if med is None:
            exit(1)
